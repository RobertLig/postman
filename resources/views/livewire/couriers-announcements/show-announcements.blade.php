<div>
    <x-header title="{{ __('Couriers` announcements') }}"
        subtitle="{{ __('These are ads from people who would like to deliver something for someone.') }}" separator>
    ...
    <div class="grid sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-5">
        @foreach ($couriers as $courier)
            <x-card :title="$courier->translate($language->id)->thing" shadow separator progress-indicator="delete({{ $courier->id }})">
                {{-- :key="$courier->id" --}}
                ...
                <div class="flex items-center justify-between gap-3 mt-2">
                    ...
                    <div>{!! Str::limit(
                        $courier->posting_day .
                            ' ' .
                            $courier->translate($language->id)->posting_month .
                            ' ' .
                            $courier->posting_year .
                            ' ' .
                            $courier->posting_hour .
                            ':' .
                            ($courier->posting_minute < 10 ? '0' . $courier->posting_minute : $courier->posting_minute),
                        30,
                    ) !!}</div>
                </div>
                ...
                <div class="flex items-center justify-between gap-3 mt-2">
                    <x-badge :value="__('on')" class="badge-soft" />
                    <div>{!! Str::limit(
                        $courier->reception_day .
                            ' ' .
                            $courier->translate($language->id)->reception_month .
                            ' ' .
                            $courier->reception_year .
                            ' ' .
                            $courier->reception_hour .
                            ':' .
                            ($courier->reception_minute < 10 ? '0' . $courier->reception_minute : $courier->reception_minute),
                        30,
                    ) !!}</div>
                </div>
