<div>
    <x-header title="{{ __('Couriers` announcements') }}" subtitle="{{ __('These are ads from people who would like to deliver something for someone.') }}" separator >

        <x-slot:actions>
            @if(auth()->user())
                <x-button label="{{ __('Create a new ad') }}" responsive icon="o-plus" link="{{ route('couriers-announcements.create') }}" class="btn btn-primary" />
            @endif
    ...
    <div class="grid sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-5"> 
        ...
        <x-card :title="$courier->translate($language->id)->thing" shadow separator progress-indicator="delete({{ $courier->id }})" > {{-- :key="$courier->id" --}}
            ...
            <div class="flex items-center justify-between gap-3 mt-2">
                ...
                <div>{!! Str::limit($courier->posting_day.' '.$courier->translate($language->id)->posting_month.' '.$courier->posting_year.' '.
                    $courier->posting_hour.':'.($courier->posting_minute < 10 ? '0'.$courier->posting_minute : $courier->posting_minute), 30) !!}</div>
            </div>

            <div class="flex items-center justify-between gap-3 mt-2">
                <x-badge :value="__('To')" class="badge-soft" />
                <div class="font-medium">{!! Str::limit($courier->translate($language->id)->reception_place, 30) !!}</div>
            </div>

            <div class="flex items-center justify-between gap-3 mt-2">
                <x-badge :value="__('on')" class="badge-soft" />
                <div>{!! Str::limit($courier->reception_day.' '.$courier->translate($language->id)->reception_month.' '.
                    $courier->reception_year.' '.$courier->reception_hour.':'.($courier->reception_minute < 10 ? '0'.
                    $courier->reception_minute : $courier->reception_minute), 30) !!}</div>
            </div>

            @can('update', $courier) 
                <x-slot:menu>
                    <x-button icon="o-pencil" class="btn-circle btn-sm" :tooltip="__('Edit')" link="{{ route('couriers-announcements.edit', ['courier' => $courier]) }}" /> 
                    <x-button icon="o-trash" class="cursor-pointer" :tooltip="__('Delete')" wire:click="delete({{ $courier->id }})" wire:confirm="{{ __('Are you sure you want to delete your ad?') }}" spinner="delete" />
                </x-slot:menu>
            @endcan 

            <x-slot:actions separator>
                <x-button :label="__('Details')" class="btn-primary" link="{{ route('couriers-announcements.show', ['courier' => $courier]) }}" />
            </x-slot:actions>
        </x-card>
        @endforeach 
    </div>
   
    {{ $couriers->onEachSide(0)->links('vendor.livewire.postman-pagination'/*, ['scrollTo' => false]*/) }}
