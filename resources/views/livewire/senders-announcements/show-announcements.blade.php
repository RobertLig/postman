<div>
    ...
    <div class="grid sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-5"> 
        ...
        <x-card :title="$senderannouncement->translate($language->id)->thing" shadow separator progress-indicator="delete({{ $senderannouncement->id }})" :key="$senderannouncement->id" > 
            ...
            <x-slot:figure>
                <img src="{{ $senderannouncement->library !== null && $senderannouncement->library->first() ? $senderannouncement->library->first()['url'] : Storage::url('senders-announcements/no-photo.jpg') }}" class="w-[500px] h-[200px] object-contain"/> {{-- object-cover |  https://picsum.photos/500/200 --}}
            </x-slot:figure>
