<aside id="cookies-policy" class="cookies cookies--no-js" data-text="{{ json_encode(__('cookieConsent::cookies.details')) }}">
    <div class="cookies__alert !bg-base-100">
        <div class="cookies__container">
            <div class="cookies__wrapper">
                <h2 class="cookies__title !text-base-content">@lang('cookieConsent::cookies.title')</h2>
                ...
                <div class="cookies__actions">
                    @cookieconsentbutton(action: 'accept.essentials', label: __('cookieConsent::cookies.essentials'), attributes: ['class' => ' cookiesBtn--essentials btn btn-info']) {{-- cookiesBtn --}}
                    @cookieconsentbutton(action: 'accept.all', label: __('cookieConsent::cookies.all'), attributes: ['class' => ' cookiesBtn--accept btn btn-info']) {{-- cookiesBtn --}}
                </div>
            </div>
        </div>
            <a href="#cookies-policy-customize" class="cookies__btn cookies__btn--customize !text-base-content">
                <span>@lang('cookieConsent::cookies.customize')</span>
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                    ...
                    <div class="cookies__section">
                        <label for="cookies-policy-check-{{ $category->key() }}" class="cookies__category">
                            ...
                            <span class="cookies__box">
                                <strong class="cookies__label !text-base-content">{{ $category->title }}</strong>
                            </span>
                            @if($category->description)
                                <p class="cookies__info !text-base-content">{{ $category->description }}</p>
                            @endif
                        </label>
                        ...
                        <div class="cookies__expandable" id="cookies-policy-{{ $category->key() }}">
                            <ul class="cookies__definitions">
                                ...
                                <li class="cookies__cookie">
                                    <p class="cookies__name !text-base-content/60">{{ $cookie->name }}</p>
                                    <p class="cookies__duration !text-base-content/60">{{ Carbon\Carbon::now()->diffForHumans(Carbon\Carbon::now()->addMinutes($cookie->duration), true) }}</p>
                                    @if($cookie->description)
                                        <p class="cookies__description !text-base-content/60">{{ $cookie->description }}</p>
                                    @endif
                                </li>
                                @endforeach
                            </ul>
                        </div>
                        <a href="#cookies-policy-{{ $category->key() }}" class="cookies__details !text-base-content">@lang('cookieConsent::cookies.details.more')</a>
                    </div>
                    @endforeach
                </div>
                <div class="cookies__save">
                    <button type="submit" class=" btn btn-info w-full">@lang('cookieConsent::cookies.save')</button>
                </div>
            </form>
        </div>
    </div>
